...
tsl::elm::Element* GuiMods::createUI() {
  ...
  std::vector<std::string_view> mods = controller.loadMods(this->source, this->group);
  ...
  for (const std::string_view &mod : mods) {
    auto item = new tsl::elm::ToggleListItem(std::string(mod), mod == activeMod);

    item->setStateChangedListener([this, mod](bool state) {
      if (state) {
        controller.deactivateMod(this->source, this->group);
        controller.activateMod(this->source, this->group, std::string(mod));
      } else {
